...
Notice that 
\begin{equation}
    \int_{t_i}^{t_f}\dd{t}(1-\cos{\theta})\dot{\phi}=\int_{t_i}^{t_f}\dd{t}(1-\cos{\theta})\frac{\dd{\phi}}{\dd{t}}\dd{t}=\int_{\phi_i}^{\phi_f}(1-\cos{\theta})\dd{\phi}
\end{equation}
and this integral represents the area on the unit sphere enclosed by the path of $\vb{\Omega}(t)$.
Therefore, the first term is 
\begin{equation}
    \hbar S \int_{t_i}^{t_f}\dd{t}(1-\cos{\theta})\dot{\phi}=\hbar S  \mathcal{A}.
\end{equation}
However, there is a subtlety here since the oriented area $\mathcal{A}$ depends on the choice of the surface bounded by the path.
If we choose the other surface bounded by the path, the oriented area would be $\mathcal{A}-4\pi$.
Therefore, if $\me^{\ii S\mathcal{A}}$ is to be single-valued, we must have
\begin{equation}
    \me^{\ii S\mathcal{A}}=\me^{\ii S(\mathcal{A}-4\pi)}
\end{equation}
...
\begin{equation}\label{transfer_matrix_1}
    T=\mqty(
        \me^{K+H} & \me^{-K}\\
        \me^{-K} & \me^{K-H}
    ).
...
Therefore, we have 
\begin{equation}
    \expval{s_i s_{i+r}}-\expval{s_i}\expval{s_{i+r}} \propto \left(\frac{\lambda_-}{\lambda_+}\right)^{|r|}.   
\end{equation}
The correlation length is defined by 
\begin{equation}
    \xi=\left(\ln{\frac{\lambda_+}{\lambda_-}}\right)^{-1}= \left(\ln{\frac{\cosh{H}+\sqrt{\sinh^2{H}+\me^{-4K}}}{\cosh{H}-\sqrt{\sinh^2{H}+\me^{-4K}}}}\right)^{-1}.
\end{equation}
and when $h=0$, we have
\begin{equation}
    \xi^{-1}=\left(\ln{\coth{K}}\right).
\end{equation}
\subsection*{(h)}
The partition function is 
\begin{equation}
    Z_N=\sum_{\{s_i\}}\me^{-\beta H}=\sum_{\{s_i\}}\prod_{i=1}^N \me^{K s_i s_{i+1}}.
\end{equation}
After integrating out the spins at sites $3i-2$ and $3i$, the terms that involve $s_2$ and $s_5$ can be expressed as
\begin{equation}\label{eq:RG_1}
    \me^{K' s_2 s_5}=\sum_{s_3,s_4} \me^{K ( s_2 s_3+s_3 s_4 + s_4 s_5)}
\end{equation}
Using the identify $\me^{K s_i s_j}=\cosh(K) + \sinh(K) s_i s_j$, we can write
\begin{equation}\label{eq:RG_2}
    \begin{split}
        \sum_{s_3,s_4} \me^{K ( s_2 s_3+s_3 s_4 + s_4 s_5)} &= \sum_{s_3,s_4}(\cosh{K}+\sinh{K} s_2 s_3)(\cosh{K}+\sinh{K} s_3 s_4)(\cosh{K}+\sinh{K} s_4 s_5)\\
                                                            &=\sum_{s_3,s_4}\cosh^3 K(1+\tanh{K} s_2 s_3)(1+\tanh{K} s_3 s_4)(1+\tanh{K} s_4 s_5)\\
                                                            &=4\cosh^3 K(1+\tanh^3{K} s_2 s_5)\\
    \end{split}
\end{equation}
Compare \cref{eq:RG_1} and \cref{eq:RG_2} up to first order and ignore the constant shift, we have
\begin{equation}
    \tanh{K'}=\tanh^3{K}.
\end{equation}
Using the RG recursion relation, we can find the fixed points by solving
\begin{equation}
    \tanh{K^*}=\tanh^3{K^*}.
\end{equation}
The solutions are $K^*=0$ and $K^*=\infty$.

\subsection*{(i)}
From \cref{transfer_matrix_1}, when $H=0$, we have
\begin{equation}
    T=\mqty(
        \me^{K} & \me^{-K}\\
        \me^{-K} & \me^{K}
...
\end{equation}
where $\tanh\tilde{K}=\me^{-2K}$ and $C$ is a constant.
After using $\sigma_x^2=1$, we have 
\begin{equation}
    \begin{split}
        \me^{\tilde{K}\sigma_x}&=1+\tilde{K}\sigma_x+\frac{\tilde{K}^2}{2!}\sigma_x^2+\frac{\tilde{K}^3}{3!}\sigma_x^3+\dots\\
                               &=\left(1+\frac{\tilde{K}^2}{2!}+\frac{\tilde{K}^4}{4!}+\dots\right)+\left(\tilde{K}+\frac{\tilde{K}^3}{3!}+\frac{\tilde{K}^5}{5!}+\dots\right)\sigma_x\\
                               &=\cosh{\tilde{K}}+\sinh{\tilde{K}}\sigma_x.
    \end{split}
\end{equation}

In the 2D case, the transfer matrix along the $y$-direction can be written as
\begin{equation}
    T_{\{s\},\{s'\}}\propto\exp\left[K\sum_{i=1}^N s_i s'_i + K \sum_{i=1}^N s'_i s'_{i+1}\right].
\end{equation}
We can rewrite the transfer matrix as
\begin{equation}
    T\propto\exp\left(K \sum_{i=1}^N s_i s'_i\right) \exp\left(K \sum_{i=1}^N s'_i s'_{i+1}\right).
\end{equation}
The first term can be identified with $\exp(\tilde{K}\sum_j \sigma^x_j)$ and the second term can be identified with $\exp(K\sum_j \sigma^z_j \sigma^z_{j+1})$.
Notice that
\begin{subequations}
    \begin{align}
        \exp(\tilde{K}\sum_j \sigma^x_j)_{\{s\},\{s'\}}&=\exp(\tilde{K}\sum_i s_i s'_{i})\\
        \exp(K\sum_j \sigma^z_j \sigma^z_{j+1})_{\{s\},\{s'\}}&=\exp(K\sum_i s_i s_{i+1})\delta_{\{s\},\{s'\}}
    \end{align}
\end{subequations}
Thus, we can verify that 
\begin{equation}
    \begin{split}
        T_{\{s\},\{s'\}}&\propto \sum_{\{s''\}}\exp(\tilde{K}\sum_j \sigma^x_j)_{\{s\},\{s''\}} \exp(K\sum_j \sigma^z_j \sigma^z_{j+1})_{\{s''\},\{s'\}}\\
                        &\propto T_{\{s\},\{s'\}}\propto\exp\left[K\sum_{i=1}^N s_i s'_i + K \sum_{i=1}^N s'_i s'_{i+1}\right].
    \end{split}
\end{equation}
...
The partition function is 
\begin{equation}
    Z=\sum_{\{s_i=\pm1\}}\exp[\frac{\beta J}{2}\sum_{i,j}C_{ij}s_i s_j+\beta h \sum_i s_i]. 
\end{equation}
After defining $K\equiv \beta J C$, the partition function can be written as
\begin{equation}
    Z=\sum_{\{s_i=\pm1\}}\exp[\frac{1}{2}\sum_{i,j}K_{ij}s_i s_j+\beta h \sum_i s_i]=\sum_{\{s\}}\exp(\frac{1}{2}s^T K s+\beta h^T s).
\end{equation} 
\subsection*{(2)}
After using the Guassian integral identity, we have
\begin{equation}
    \exp(\frac{1}{2} s^T K s)=\mathcal{N}\int \dd{\phi}\exp(-\frac{1}{2}\phi^T K \phi + s^T  \phi).
\end{equation}
Thus, the partition function can be written as
\begin{equation}
    \begin{split}
        Z&=\mathcal{N}\int \dd{\phi}\exp(-\frac{1}{2}\phi^T K \phi)\sum_{\{s\}}\exp(s^T(\phi+\beta h))\\
         &=\mathcal{N}\int \dd{\phi}\exp(-\frac{1}{2}\phi^T K \phi)\sum_{s_i=\pm 1} \prod_{i=1}^N \exp(s_i(\phi_i+\beta h))\\
         &=\mathcal{N}\int \dd{\phi}\exp(-\frac{1}{2}\phi^T K \phi)\prod_{i=1}^N \sum_{s_i=\pm 1}\exp(s_i(\phi_i+\beta h))\\
         &=\mathcal{N}\int \dd{\phi}\exp(-\frac{1}{2}\phi^T K \phi)\prod_{i=1}^N 2\cosh(\phi_i+\beta h)\\
         &=\mathcal{N}\int \dd{\phi}\exp\left(-\frac{1}{2}\phi^T K \phi +\sum_{i=1}^N \ln{[2\cosh(\phi_i+\beta h)]}\right).
    \end{split}
\end{equation}
In order to have a positive definite $K$, we require that all eigenvalues of $K$ are positive, which can be done by adding a sufficiently large positive constant to the diagonal elements of $K$:
\begin{equation}
    K\to K+\epsilon \mathbb{I}, \quad \epsilon>\min(\lambda_i\text{ of }K)\quad\text{if } \min(\lambda_i\text{ of }K)<0.
\end{equation}
\subsection*{(3)}
If we write 
\begin{equation}
    Z=\int\mathcal{\phi}\exp(-S_{\text{eff}}[\phi]),
\end{equation}
we can identify the effective action as
\begin{equation}
    S_{\text{eff}}[\phi]=\frac{1}{2}\phi^T K \phi -\sum_{i=1}^N \ln{[2\cosh(\phi_i+\beta h)]}+\text{const}.
\end{equation}
The first term in $S_{\text{eff}}[\phi]$ is a quadratic form in $\phi$ and it is non-local since there will be terms like $\phi_i\phi_k$ with $i\neq k$.
The second term is a sum of logarithmic functions which is local in $\phi$.
\subsection*{(4)}
Since the system is translationally invariant, we have the following Fourier transforms 
\begin{subequations}
    \begin{align}
        \phi_i&=\frac{1}{\sqrt{N}}\sum_{k\in\text{BZ}}\me^{\ii k\cdot r_i}\phi_k,\\
        C_{ij}&=\frac{1}{N}\sum_{k\in\text{BZ}}\me^{\ii k\cdot (r_i-r_j)}C(k).
    \end{align}
\end{subequations}
The inverse of $K$ in momentum space is now
\begin{equation}
    K^{-1}(k)=\frac{1}{\beta J C(k)}
\end{equation}
and the quadratic term in the effective action can be written as
\begin{equation}
    \begin{split}
        \frac{1}{2}\sum_{i,j}\phi_i K^{-1}_{ij} \phi_j=\frac{1}{2}\sum_{k\in\text{BZ}}\phi_{-k} K^{-1}(k) \phi_k=\frac{1}{2\beta J}\sum_{k\in\text{BZ}}\frac{1}{C(k)}\phi_{-k}\phi_k=\frac{1}{2\beta J}\sum_{k\in\text{BZ}}\frac{1}{C(k)}|\phi_k|^2.
    \end{split}
\end{equation}
When $C(k)=0$ for some $k$, then a regularization is needed and the quadratic term can be modified as
\begin{equation}
    \frac{1}{2\beta J}\sum_{k\in\text{BZ}}\frac{1}{C(k)+\epsilon}|\phi_k|^2, \quad \epsilon>0.
\end{equation}
\subsection*{(5)}
The expansion of $C(k)$ around $k=0$ is 
\begin{equation}
    C(k)=C(0)-\alpha a^2 k^2=z-\alpha a^2 k^2.
\end{equation}
Thus, we have the small-$k$ approximation 
\begin{equation}
    \frac{1}{\beta J C(k)}\approx\frac{1}{\beta J z}\frac{1}{1-(\alpha/z)a^2 k^2}\approx \frac{1}{\beta J z}\left(1+\frac{\alpha a^2}{z}k^2\right).
\end{equation}
After transforming back to real space, the quadratic term in the effective action becomes
\begin{equation}
    \begin{split}
        \frac{1}{2}\sum_{i,j}\phi_i K^{-1}_{ij} \phi_j&\approx\frac{1}{2\beta J z}\sum_{k\in\text{BZ}}\left(1+\frac{\alpha a^2}{z}k^2\right)|\phi_k|^2\\
        &=\frac{1}{2\beta J z}\int \dd{^d x}\left(\phi(x)^2+\frac{\alpha a^2}{z}(\nabla \phi(x))^2\right)=\int \dd[d]{x}\left[\frac{r_0}{2}\phi(x)^2+\frac{\kappa}{2}(\grad{\phi(x)})^2\right]
    \end{split}
\end{equation}
...
\begin{subequations}
    \begin{align}
        r_0&=\frac{1}{\beta J z},\\
        \kappa&=\frac{\alpha a^2}{\beta J z^2}.
    \end{align}
\end{subequations}

\subsection*{(6)}
We can expand the logarithmic term in the effective action as
\begin{equation}
    \begin{split}
        -\sum_{i=1}^N \ln{[2\cosh(\phi_i+\beta h)]}&=-\sum_{i=1}^N \ln{2}-\sum_{i=1}^N \ln{\left[1+\frac{(\phi_i+\beta h)^2}{2!}+\frac{(\phi_i+\beta h)^4}{4!}+\dots\right]}\\
        &\approx -\sum_{i=1}^N \ln{2}-\sum_{i=1}^N \left[\frac{(\phi_i+\beta h)^2}{2}-\frac{(\phi_i+\beta h)^4}{12}+\dots\right]
    \end{split}
\end{equation}
After combining with the quadratic term, the effective action can be written as
\begin{equation}
    \begin{split}
        S_{\text{eff}}[\phi]&=\int \dd[d]{x}\left[\frac{\kappa}{2}(\grad{\phi(x)})^2+\frac{r}{2}\phi(x)^2 +\frac{u}{4!} \phi(x)^4 -H_{\text{eff}}\phi(x)\right]+\text{const}
    \end{split}
\end{equation}
where we have defined
\begin{subequations}
    \begin{align}
        r&=r_0-1+\beta^2 h^2,\\
        u&=2,\\
        H_{\text{eff}}&=\beta h-\frac{\beta^3 h^3}{3}.
    \end{align}
\end{subequations}
$r$ and $\kappa$ are the coefficients of the mass term and the gradient term of the low-energy effective action, respectively, which are related to the microscopic parameters of the original Ising model.
